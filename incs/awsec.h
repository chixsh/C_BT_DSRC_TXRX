...
#define CMD_ERR_INVALID_TIME                                0x64
...
typedef struct {
    ...
    SINT32 local_longitude;
    //use variable or use htonl() call after computation for location
    double distance;
} location;
...
typedef enum {
    ...
    signed_wsa = 11, certificate_response_acknowledgment = 12,
    crl_req = 236,
    crl_req_error = 237,
...
EC_POINT *PrivateKeyReconstruct(UINT8 *pHash, UINT8 *pPrivKeyReconBuf, UINT8 *pEphemeralPrivKey, UINT8 *pPrivateKey,
                                EC_GROUP *pGroup);

EC_POINT *PublicKeyReconstruct(UINT8 *pHash, UINT8 *pPubKeyRecon, UINT8 *pCAPubKeyRecon, UINT8 *pStaticPubKey,
                               EC_GROUP *pGroup);
...
UINT32 AWSecEncrypt(UINT32 cipherAlgo, UINT8 *pCipherKey, UINT8 *pIVNonce, UINT32 uiNonceLen, UINT8 *pBuf,
                    UINT32 uiBufLen, UINT8 *pCipherText, UINT32 *uiCipherTextLen, UINT32 uiTagLen);

UINT32 AWSecDecrypt(UINT32 cipherAlgo, UINT8 *pCipherKey, UINT8 *pIVNonce, UINT32 uiNonceLen, UINT8 *pCipherText,
                    UINT32 uiCipherTextLen, UINT8 *pBuf, UINT32 *uiBufLen, UINT32 uiTagLen);

UINT8 *AWSecHash(UINT32 hashAlgo, UINT8 *pBuf, UINT32 uiBufLen, UINT8 *pDigest, UINT32 *uiDigestLen, UINT32 uiReqLen);

UINT32 GetSignedData(UINT8 *pPrivKey, UINT8 *pData, UINT32 uiDataLen, UINT8 *pDigest, UINT32 uiDigestLen,
                     UINT8 *pSignDataBuf);
