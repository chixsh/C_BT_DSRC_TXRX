...
struct GenericDataLong{
    ...
    //char     		contents[FOURK];   /* BA- TBD proper fix for 4k wsmp support */
    char     		contents[HALFK];
} ;
...
typedef struct {
    ...
    /*channel access type
                                   ...
                                   0 -> provider access sch in sch interval
                                          (alternating access) */
    u_int8_t repeatrate;
    /*number of VSA frames to be transmitted per 5
                                seconds. ignored in case of unicast frames.*/
    u_int8_t ipservice;       //TRUE / FALSE
    struct in6_addr ipv6addr;
    u_int16_t serviceport;
    u_int8_t rcpithres;
    /*Value in dBm below which, the service
    ...
    /*1: auto access. allows SCH access if it matches
...
struct waveHandler
{
    u_int16_t length;
    u_int16_t type;
    //char data[FOURK+104];  /* BA- TBD proper fix for 4k wsmp support */
    char data[1400];
};
#else
struct waveHandler {
